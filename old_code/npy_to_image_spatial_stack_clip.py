...
def npy_to_image(input_files, height):
    ...
    for input_file in input_files:
        ...
        for i in range(int(84000/height)):
            ...
            plt.axis('off')
            plt.imshow(data[a:a+height,:96], aspect="1", cmap='gray_r')
            plt.savefig('./image_96_clip/'+name+'.png', format='png', bbox_inches='tight', pad_inches=0)
            plt.clf()
        b += 1
        print(b/len(input_files)*100,"%d complite")
    return
desired_pixel = 125
dpi = 100
